...
const UpdateUser: React.FC = ({route}) => {
  ...
  const [surname, setSurname] = useState(userInfo.name);
  const [age, setAge] = useState(userInfo.name);
  const [phone, setPhone] = useState(userInfo.name);
  const [email, setEmail] = useState(userInfo.name);
  const [city, setCity] = useState(userInfo.name);
